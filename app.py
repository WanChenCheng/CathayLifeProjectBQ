...
# =========================
if page == "1. 上傳檔案與時間篩選":
    st.markdown("### 步驟 1: 從 BigQuery 載入資料並進行時間篩選")
    ...
    # 按鈕載入 BigQuery 資料
    if st.button("從 BigQuery 載入資料"):
        try:
            query = load_sql_file("behavior_model_input.sql")
            client.query(query).result()

            df = client.query("SELECT * FROM `lab-martech-cxl.Demo.BehaviorModelInput`").to_dataframe()
            st.session_state.raw_uploaded_data = df
            st.success(f"資料已成功從 BigQuery 載入，共 {len(df)} 筆")
        except Exception as e:
            st.error(f"載入資料錯誤：{e}")
            st.session_state.raw_uploaded_data = None

    # ======= 後續欄位檢查 + 日期篩選 =======
    if st.session_state.get("raw_uploaded_data") is not None:
